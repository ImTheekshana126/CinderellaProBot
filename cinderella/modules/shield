...
from cinderella import BOT_ID

# from YoneRobot.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from cinderella.pyrogramee.telethonbasics import is_admin
from cinderella.events import register
from cinderella import MONGO_DB_URI 
from pymongo import MongoClient
from cinderella.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, JNC deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`Automatic Detections Powered By JNC AI` \n**#GROUP_GUARDIAN** "
...
"""
@tbot.on_message(filters.command("nsfwguardian") & ~filters.edited & ~filters.bot)
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, JNC deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`Automatic Detections Powered By JNC AI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
            ...
            if event.chat_id == c["id"]:
                if better_profanity.profanity.contains_profanity(msg):
                    ...
                    if sender.username is None:
                        ...
                        final = (
                              f"__**#PROFANITY**__ 🪓😤\n" 
                              f"**USERNAME** **:**[{st}](tg://user?id={hh})\n" 
                              f"**USER ID** **:**{hh}\n" 
                              f"**word** **=** **{msg}** is detected as a slag word ⚠️\n" 
                              f"So your message has been deleted !"
                        )
                    else:
                        st = sender.first_name
                        hh = sender.id
        ...
        if event.photo:
            if event.chat_id == c["id"]:
                ...
                if nude.is_nude("./nudes.jpg"):
                    ...
                    st = sender.first_name
                    hh = sender.id
                    final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, JNN deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`Automatic Detections Powered By JNC AI` \n**#GROUP_GUARDIAN** "
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
                    os.remove("nudes.jpg")
...
__help__ = """
<b> Group Guardian: </b>
🦊 𝔽𝕠𝕩 𝕏 𝔹𝕠𝕥 can protect your group from NSFW senders, Slag word users and also can force members to use English
<b>Commmands</b>
 🔱 /antinsfw <i>enable/disable</i> - Enable|Disable Porn cleaning
 🔱 /profanity <i>on/off</i> - Enable|Disable slag word cleaning
 🔱 /addprofanity (reply to any message) - To report that message as profanity word
